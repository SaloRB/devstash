...
# Rewrite last N commits (squash, reword, etc.)
git rebase -i HEAD~<N>
...
git branch --merged main | grep -v "^\\* main" | xargs git branch -d`,
        description: "Advanced git commands: reset, rebase, bisect, branch cleanup",
        userId: user.id,
        itemTypeId: commandType.id,
      },
    }),
    prisma.item.upsert({
      where: { id: "seed-term-2" },
      update: {},
      create: {
        id: "seed-term-2",
...
docker stats --no-stream`,
        description: "Docker container and image management one-liners",
        userId: user.id,
        itemTypeId: commandType.id,
      },
    }),
    prisma.item.upsert({
      where: { id: "seed-term-3" },
      update: {},
      create: {
        id: "seed-term-3",
...
# Run a command and keep it alive after logout
nohup <command> &

# Monitor real-time CPU/memory by process
top -o cpu

# List background jobs in current shell
jobs -l`,
        description: "Process inspection and management: lsof, pkill, nohup, top",
        userId: user.id,
        itemTypeId: commandType.id,
      },
    }),
    prisma.item.upsert({
      where: { id: "seed-term-4" },
      update: {},
      create: {
        id: "seed-term-4",
...
# List globally installed packages (one level deep)
npm list -g --depth=0`,
        description: "npm commands for auditing, pruning, and inspecting dependencies",
        userId: user.id,
        itemTypeId: commandType.id,
      },
    }),
  ...
  await Promise.all(
    terminalItems.map((item) =>
      prisma.itemCollection.upsert({
        where: { itemId_collectionId: { itemId: item.id, collectionId: terminal.id } },
        update: {},
        create: { itemId: item.id, collectionId: terminal.id },
      })
    )
  );
  ...
  const designItems = await Promise.all([
    ...
    prisma.item.upsert({
      ...
      create: {
        ...
        description: "Copy-paste React component library built on Radix UI",
        userId: user.id,
        itemTypeId: linkType.id,
      },
    }),
    prisma.item.upsert({
      where: { id: "seed-design-3" },
      update: {},
      create: {
        id: "seed-design-3",
  ...
  await Promise.all(
    designItems.map((item) =>
      prisma.itemCollection.upsert({
        where: { itemId_collectionId: { itemId: item.id, collectionId: design.id } },
        update: {},
        create: { itemId: item.id, collectionId: design.id },
      })
    )
  );
